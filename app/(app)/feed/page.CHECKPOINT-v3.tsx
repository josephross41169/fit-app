    ...
    <div style={{ position:"fixed",inset:0,zIndex:9999,background:"rgba(0,0,0,0.95)",display:"flex",alignItems:"center",justifyContent:"center" }}>
      ...
      <div style={{ width:"100%",maxWidth:400,aspectRatio:"9/16",borderRadius:20,overflow:"hidden",position:"relative",background:"#1A1A2E" }}>
        {story.photo
          ...
          : <div style={{ width:"100%",height:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:`linear-gradient(135deg,${C.blue},#7DD8EA)` }}>
...
function SideWorkout({ workout }: { workout: NonNullable<Post["workout"]> }) {
  ...
  return (
    <div style={{ borderRadius:14,overflow:"hidden",border:`1px solid ${C.darkBorder}`,marginBottom:10 }}>
      <button onClick={() => setOpen(o => !o)} style={{ width:"100%",background:"linear-gradient(135deg,#4BBFD6,#3AA8C1)",padding:"13px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",border:"none",cursor:"pointer",textAlign:"left" }}>
        <div style={{ display:"flex",alignItems:"center",gap:10 }}>
        ...
        <div style={{ background:"#1E2235",padding:"10px 14px" }}>
          {workout.exercises.length > 0 && (<>
            ...
            {workout.exercises.map((ex,i) => (
              <div key={i} style={{ display:"grid",gridTemplateColumns:"1fr 42px 42px 72px",gap:5,padding:"7px 4px",borderRadius:7,background:i%2===0?"rgba(75,191,214,0.08)":"transparent" }}>
          ...
          {workout.cardio.length > 0 && (
            <div style={{ marginTop:workout.exercises.length?10:0 }}>
              ...
              {workout.cardio.map((c,i) => (
                <div key={i} style={{ display:"grid",gridTemplateColumns:"1fr 70px 70px",gap:5,padding:"7px 4px",borderRadius:7,background:i%2===0?"rgba(75,191,214,0.08)":"transparent" }}>
  ...
  return (
    <div style={{ background:C.darkCard, borderRadius:18, border:`1px solid ${C.darkBorder}`, overflow:"hidden", marginBottom:16 }}>
      ...
      <div style={{ display:"flex",alignItems:"center",gap:12,padding:"14px 16px 12px",borderBottom:`1px solid ${C.darkBorder}` }}>
        <div style={{ width:44,height:44,borderRadius:"50%",background:"linear-gradient(135deg,#4BBFD6,#3AA8C1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,fontWeight:900,color:"#fff",flexShrink:0 }}>
          {post.avatar}
        </div>
        <div>
...
function PostCard({ post, onUpdate }: { post: Post; onUpdate: (p: Post) => void }) {
  ...
  return (
    <>
      ...
      <div style={{ background:C.white, borderRadius:20, border:`2px solid ${C.greenMid}`, boxShadow:"0 4px 24px rgba(75,191,214,0.10)", marginBottom:24, overflow:"hidden" }}>
          ...
          <div style={{ padding:"0 18px 10px",display:"flex",flexDirection:"column",gap:10 }}>
            {visibleComments.map(c => (
              <div key={c.id} style={{ display:"flex",gap:10,alignItems:"flex-start" }}>
                <div style={{ width:32,height:32,borderRadius:"50%",background:`linear-gradient(135deg,${C.blue},#7DD8EA)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:900,color:"#fff",flexShrink:0 }}>{c.avatar}</div>
        ...
        <div style={{ padding:"0 18px 16px",display:"flex",gap:10,alignItems:"center" }}>
          <div style={{ width:32,height:32,borderRadius:"50%",background:`linear-gradient(135deg,${C.blue},#7DD8EA)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:900,color:"#fff",flexShrink:0 }}>JB</div>
          ...
          {SUGGESTED_USERS.map(u => (
            <div key={u.id} style={{ background:C.darkCard,borderRadius:18,border:`1px solid ${C.darkBorder}`,overflow:"hidden",marginBottom:16 }}>
              ...
              <div style={{ padding:"10px 16px",borderBottom:`1px solid ${C.darkBorder}` }}>
                <button style={{ width:"100%",padding:"8px",borderRadius:10,background:"linear-gradient(135deg,#4BBFD6,#3AA8C1)",border:"none",color:"#fff",fontWeight:800,fontSize:13,cursor:"pointer" }}>
                  + Follow
                </button>
              </div>
