...
export default function CompetitionsPage() {
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: "#FFF8F0" }}>
      {/* Header */}
      <div className="sticky top-0 z-10 bg-white border-b px-4 pt-12 pb-4" style={{ borderColor: "#C9E8D8" }}>
        <h1 className="text-2xl font-black" style={{ color: "#161D19" }}>Competitions 🏆</h1>
      ...
      <div className="px-4 py-4 space-y-6">
        {/* Active */}
        <div>
          <h2 className="text-lg font-bold mb-3" style={{ color: "#161D19" }}>⚡ Active Challenges</h2>
          {active.map(comp => (
            <div key={comp.id} className="bg-white rounded-3xl p-5 shadow-sm mb-3" style={{ border: "1px solid #C9E8D8" }}>
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="flex-1">
                  <h3 className="font-bold text-base" style={{ color: "#161D19" }}>{comp.name}</h3>
        ...
        {/* Browse */}
        <div>
          <h2 className="text-lg font-bold mb-3" style={{ color: "#161D19" }}>Join a Challenge</h2>
          <div className="space-y-3">
            {browse.map(comp => (
              <div key={comp.id} className="bg-white rounded-3xl p-4 shadow-sm flex items-center gap-3" style={{ border: "1px solid #C9E8D8" }}>
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl flex-shrink-0" style={{ background: "#FFF8F0" }}>
                  {TYPE_EMOJI[comp.type]}
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-sm" style={{ color: "#161D19" }}>{comp.name}</h3>
