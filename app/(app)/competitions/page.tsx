...
export default function CompetitionsPage() {
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: "#FFF8F0" }}>
      {/* Header */}
      <div className="sticky top-0 z-10 bg-white border-b px-4 pt-12 pb-4" style={{ borderColor: "#BBF7D0" }}>
      ...
      <div className="px-4 py-4 space-y-6">
        ...
        <div>
          ...
          {active.map(comp => (
            <div key={comp.id} className="bg-white rounded-3xl p-5 shadow-sm mb-3" style={{ border: "1px solid #BBF7D0" }}>
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="text-center flex-shrink-0">
                  <div className="text-2xl font-black" style={{ color: "#16A34A" }}>{comp.daysLeft}</div>
              ...
              <div className="mb-3">
                ...
                <div className="h-3 rounded-full overflow-hidden" style={{ background: "#BBF7D0" }}>
                  <div className="h-full rounded-full transition-all" style={{ width: `${comp.progress * 100}%`, background: "linear-gradient(90deg, #16A34A, #22C55E)" }} />
                </div>
                <div className="text-right text-xs mt-1 font-semibold" style={{ color: "#16A34A" }}>{Math.round(comp.progress * 100)}%</div>
              </div>

              <div className="flex items-center justify-between">
                <span className="text-xs" style={{ color: "#6B7280" }}>👥 {comp.participants.toLocaleString()} competing</span>
                <span className="text-sm font-bold" style={{ color: "#22C55E" }}>{comp.prize}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {browse.map(comp => (
              <div key={comp.id} className="bg-white rounded-3xl p-4 shadow-sm flex items-center gap-3" style={{ border: "1px solid #BBF7D0" }}>
