...
type Exercise = { name: string; sets: string; reps: string; weight: string; weights: string[]; repsArr?: string[]; notes?: string; bodyweight?: boolean; timed?: boolean;
          ...
          {results.map((ex, i) => (
            <button
              ...
              style={{
                ...
                cursor: 'pointer', borderBottom: i < results.length - 1 ? '1px solid #1B231E' : 'none',
              }}
            >
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', marginBottom: 14 }}>
      <div style={{ position: 'relative' }}>
        <input
          style={{
            background: "#111111", border: "1.5px solid #1B231E", borderRadius: 10,
...
export default function PostPage() {
  ...
  // name) so the existing save flow that writes posts.location still works
  ...
  async function handlePost() {
    ...
    try {
      const insertRow: any = {
        ...
        // Location — when LocationPicker selected a real pin, write its id
        // AND its display name to the legacy `location` text column so old
        // queries that read posts.location still show the place name. If
        // the user typed something free-form, only the text field is set.
      ...
      // Same fallback for location_id — older DB rows / pre-migration
      // schemas don't have this column. Strip and retry; the text-only
      // location field already has the place name.
  ...
  if (saved) {
    ...
    return (
      <div style={{ background: C.bg, minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, padding: "0 24px" }}>
        ...
        {hasPRs && (
          <div style={{ background: "#111811", borderRadius: 18, padding: "16px 20px", width: "100%", maxWidth: 380, border: "2px solid #F5A623" }}>
            {newPRs.map((pr, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: i < newPRs.length - 1 ? "1px solid #1B231E" : "none" }}>
            ...
            <div>
              ...
              {TAB_DEFS.map(t => (
                <button key={t.key} onClick={() => setLogTab(t.key)} style={{
                  ...
                  background: logTab === t.key ? t.color : "#111811",
                  color: logTab === t.key ? "#fff" : C.sub,
                  transition: "all 0.15s",
                  ...
                  {exercises.map((ex, i) => {
                    ...
                    <div key={i} style={{ background: "#0D0D0D", borderRadius: 16, padding: 14, border: `1px solid ${C.greenMid}` }}>
                      ...
                      {/* Timed toggle — planks, carries, dead hangs, intervals:
                          ...
                          the same per-set slots; `timed` flags how to read them. */}
                      <button type="button"
                      ...
                      <label style={{ fontSize: 10, fontWeight: 800, color: C.sub, display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.8 }}>{ex.timed ? (ex.bodyweight ? "Time in seconds (per set)" : "Seconds × Weight (per set)") : (ex.bodyweight ? "Reps (per set)" : "Reps × Weight (per set)")}</label>
                          ...
                          return (
                            <div key={s} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                              ...
                              <input
                                ...
                                type="text" inputMode="numeric" placeholder={ex.timed ? "sec" : "reps"}
                                value={(ex.repsArr || [])[s] ?? ''}
                ...
                {foodItems.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {foodItems.map((item, i) => (
                      <div key={i} style={{ background: '#0D0D0D', borderRadius: 14, padding: '10px 12px', border: '1px solid #1B231E' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                          ...
                          {(() => {
                            ...
                            return (
                              <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50, marginTop: 4, background: '#111811', border: '1px solid #2E2440', borderRadius: 10, boxShadow: '0 10px 30px rgba(0,0,0,0.5)', overflow: 'hidden' }}>
                                {matches.map((s) => (
                                  <button
                                    key={s.id}
                                    onMouseDown={() => {
                          ...
                          <button
                            ...
                            style={{ width: 30, height: 30, borderRadius: "50%", border: "none", background: "#1B231E", color: "#F5A623", fontSize: 15, cursor: "pointer", flexShrink: 0 }}>☆</button>
                ...
                {user ? (
                  <LocationPicker
                    ...
                    onChange={(loc) => {
                      ...
                      // Mirror the picked place's name into the legacy text
                      // field. If user clears the picker, leave the text
                      // alone in case they typed something free-form first.
                      if (loc) setLocation(loc.name);
                    }}
                    userId={user.id}
                    defaultCity={(user as any)?.profile?.city || ""}
                  />
                ) : (
                  <input
                    style={iStyle}
                    placeholder="📍 Add location (e.g. Las Vegas, NV)"
                    value={location}
