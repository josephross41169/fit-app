              ...
              <div style={{ background: C.white, borderRadius: 22, padding: 20, border: `2px solid ${C.greenMid}` }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  ...
                  <div>
                    ...
                    <input style={iStyle} type="number" placeholder="e.g. 450" value={woCalories} onChange={e => setWoCalories(e.target.value)} />
                  </div>
                </div>
              </div>
                ...
                {exercises.length === 0 ? (
                  ...
                  {exercises.map((ex, i) => (
                    <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 60px 60px 80px 36px", gap: 8, marginBottom: 8, alignItems: "center" }}>
                      ...
                      <input style={iStyle} type="number" value={ex.sets} onChange={e => setExercises(exs => exs.map((x, j) => j === i ? { ...x, sets: e.target.value } : x))} />
                      <input style={iStyle} type="number" value={ex.reps} onChange={e => setExercises(exs => exs.map((x, j) => j === i ? { ...x, reps: e.target.value } : x))} />
              ...
              <div style={{ background: C.white, borderRadius: 22, padding: 20, border: `2px solid ${C.greenMid}` }}>
                ...
                {foodItems.length === 0 ? (
                  ...
                  <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 90px 36px", gap: 8, marginBottom: 8, alignItems: "center" }}>
                    ...
                    <input style={iStyle} type="number" placeholder="kcal" value={item.calories} onChange={e => setFoodItems(f => f.map((x, j) => j === i ? { ...x, calories: e.target.value } : x))} />
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 12 }}>
                  {[{ l: "Protein (g)", v: protein, s: setProtein }, { l: "Carbs (g)", v: carbs, s: setCarbs }, { l: "Fat (g)", v: fat, s: setFat }].map(f => (
                    <div key={f.l}>
                      ...
                      <input style={iStyle} type="number" placeholder="0" value={f.v} onChange={e => f.s(e.target.value)} />
                    </div>
                  ))}
                </div>
