...
import { share, appUrl } from "@/lib/share";
...
export default function GroupChallengesPage() {
  ...
  async function shareInvite(token: string) {
    ...
    const outcome = await share({
      url: appUrl(`/challenge/${token}`),
      title: "Join my Livelee challenge",
      text: "I started a fitness challenge — come compete with me!",
    });
    // Per-token copied tracking: this page can show "✓ Copied" on the
    // specific challenge button that was clicked, not all of them. We
    // can't use shareWithToast which assumes a single boolean setter.
    if (outcome === "copied") {
