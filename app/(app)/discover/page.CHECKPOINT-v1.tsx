...
// ── Local Event Card — purple pill style ─────────────────────────────────────
function EventCard({ event }: { event: typeof LOCAL_EVENTS[0] }) {
  ...
  return (
    ...
    onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform = "translateY(-2px)"; (e.currentTarget as HTMLDivElement).style.boxShadow = "0 8px 24px rgba(124,58,237,0.4)"; }}
    onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.transform = ""; (e.currentTarget as HTMLDivElement).style.boxShadow = "0 4px 16px rgba(124,58,237,0.25)"; }}
    >
      <div style={{ display:"flex",alignItems:"center",gap:12,padding:"14px 16px" }}>
        {/* Date badge */}
        <div style={{ width:48,height:48,borderRadius:13,background:"rgba(255,255,255,0.2)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",flexShrink:0,border:"1.5px solid rgba(255,255,255,0.3)" }}>
        ...
        <div style={{ flex:1,minWidth:0 }}>
          <div style={{ display:"flex",alignItems:"center",gap:6,marginBottom:2 }}>
            ...
            <span style={{ fontWeight:900,fontSize:14,color:"#fff",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{event.name}</span>
          </div>
          <div style={{ fontSize:11,color:"rgba(255,255,255,0.85)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>📍 {event.venue}</div>
          <div style={{ display:"flex",gap:8,marginTop:4,alignItems:"center" }}>
            <span style={{ background:"rgba(255,255,255,0.2)",color:"#fff",fontSize:10,fontWeight:700,padding:"2px 9px",borderRadius:99 }}>{event.category}</span>
            <span style={{ color:"rgba(255,255,255,0.9)",fontSize:11,fontWeight:800 }}>{event.price}</span>
            <span style={{ color:"rgba(255,255,255,0.75)",fontSize:10 }}>· {event.time}</span>
          </div>
        </div>
        {/* Save button */}
        <button onClick={e => { e.stopPropagation(); setSaved(s=>!s); }} style={{ background:"rgba(255,255,255,0.2)",border:"1.5px solid rgba(255,255,255,0.35)",borderRadius:10,padding:"6px 8px",cursor:"pointer",color:"#fff",fontSize:14,flexShrink:0 }}>
          {saved ? "✓" : "＋"}
        </button>
      </div>
    </div>
    ...
    <div style={{ display:"flex", gap:48, alignItems:"flex-start", maxWidth:1200, margin:"0 auto", padding:"24px 24px 60px" }}>
      ...
      <div style={{ width:320, flexShrink:0 }}>
        ...
        <div style={{ marginBottom:16,paddingBottom:12,borderBottom:`1px solid ${C.darkBorder}` }}>
          <div style={{ fontWeight:900,fontSize:16,color:"#E2E8F0",marginBottom:2 }}>Local Events This Week</div>
          <div style={{ fontSize:12,color:C.darkSub }}>Las Vegas · Mar 27–30</div>
        </div>

        {LOCAL_EVENTS.map(event => <EventCard key={event.id} event={event} />)}

        {/* Submit event CTA */}
        <div style={{ marginTop:8,padding:"16px",background:C.darkCard,borderRadius:16,border:`1px dashed ${C.darkBorder}`,textAlign:"center",cursor:"pointer" }}>
      ...
      <div style={{ position:"sticky",top:0,zIndex:100,background:C.white,borderBottom:`2px solid ${C.greenLight}` }}>
        <div style={{ maxWidth:1200,margin:"0 auto",padding:"14px 24px 0",display:"flex",alignItems:"center",gap:16 }}>
          ...
          <div style={{ display:"flex",alignItems:"center",gap:8,marginRight:8 }}>
            ...
            <span style={{ fontWeight:900,fontSize:20,color:C.text }}>Discover</span>
          </div>
